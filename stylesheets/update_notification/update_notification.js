...
function u_n_close(){
    localStorage.setItem("musik_updatelist_stand", Object.keys(updatelist[0]).length);
...
function u_n_start(){
    if(localStorage.getItem("musik_updatelist_stand")){
        if(JSON.stringify(Object.keys(updatelist[0]).length) == localStorage.getItem("musik_updatelist_stand")){
            return;
        }else{
            u_n_show();
        }
